...
pub const Renderer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, window: Window) !Renderer {
        ...
        const geom_frag = wgpu.FragmentState{
            .module = geom_shader, .entry_point = "fs",
        ...
        const text_frag = wgpu.FragmentState{
            .module = text_shader, .entry_point = "fs",
            .target_count = 1, .targets = @ptrCast(&color_target),
        };
    ...
    pub fn endFrame(self: *Renderer, back_buffer: wgpu.TextureView) void {
        ...
        // Flush any new glyphs into the atlas texture before building geometry
        ...
        if (result.vertex_count > 0) {
            gctx.queue.writeBuffer(gctx.lookupResource(self.vertex_buf).?, 0,
                rc.Vertex, self.cpu_verts[0..result.vertex_count]);
            gctx.queue.writeBuffer(gctx.lookupResource(self.index_buf).?, 0,
                u32, self.cpu_idxs[0..result.index_count]);
        }
        ...
        if (result.vertex_count > 0 and result.draw_call_count > 0) {
            pass.setVertexBuffer(0, gctx.lookupResource(self.vertex_buf).?, 0,
                result.vertex_count * @sizeOf(rc.Vertex));
            pass.setIndexBuffer(gctx.lookupResource(self.index_buf).?, .uint32, 0,
                result.index_count * @sizeOf(u32));
            ...
            for (self.cpu_dcs[0..result.draw_call_count]) |dc| {
                // Scissor from clay is in logical pixels — convert to physical
    ...
    pub fn getPointerState(self: *Renderer) clay.PointerData {
        ...
        const ui = self.font_atlas.ui_scale;
        return .{
            .position = .{ .x = @as(f32, @floatCast(pos[0])) / ui,
                           .y = @as(f32, @floatCast(pos[1])) / ui },
            .state = switch (btn) {
                .press => .pressed,
                else => .released,
            },
